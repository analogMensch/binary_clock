/*
BINARY CLOCK v0.13 (2022/11/23)

- micro controller selection in rows 22,23 or 24,25
- web server port in row 174
- LED type in row 171
- data pin for LEDs in row 36
- daylight saving offset in row 119
- GMT offset in row 120
- date type in row 121
- dark mode auto reset atfer alarm in row 158
- color for standby (day mode) in row 127
- color for time (day mode) in row 128
- color for week day (day mode) in row 129
- color for date (day mode) in row 130
- color for standby (night mode) in row 131
- color for time (night mode) in row 132
- color for week day (night mode) in row 133 
- color for date (night mode) in row 134
*/
#include <Arduino.h>
        ...
        if (mode <= 11) {  //check for valid mode request
          ...
          if (mode == 4) {  //check for valid values in mode 4
            if (columnLeft > 31 || columnRight > 63) {
              ...
              change = true;
              request->send(200, "text/plain", "valid");
            }
          }
            ...
            if (columnLeft <= 23 && columnRight <= 59 && alarm2Set <= 2 && (weekMessage.length() == 0 || weekMessage.length() == 7)) {  //catch only valid values
              ...
              request->send(200, "text/plain", alarmRequest);
...
void loop() {
  if (millis() - pulse >= pulseDelay) {
    ...
    if (!change) {
      ...
      //show alarm 1
      else if (alarm1Mode > 0 && ntpHour == alarm1Left && ntpMinute == alarm1Right && ntpSecond == 0 && alarm1Week[ntpWeek - 1] > 0) {
        requestTime = alarm1Time;
        requestColor = alarm1Color;
        columnLeft = alarm1Left;
        columnRight = alarm1Right;
      ...
      //show alarm 2
      else if (alarm2Mode > 0 && ntpHour == alarm2Left && ntpMinute == alarm2Right && ntpSecond == 0 && alarm2Week[ntpWeek - 1] > 0) {
        requestTime = alarm2Time;
        requestColor = alarm2Color;
        columnLeft = alarm2Left;
        columnRight = alarm2Right;
      ...
      //synchronize clock to milliseconds every 15 minutes
      else if (ntpHour == 4 && ntpMinute == 29 && ntpSecond == 30) {
        Serial.println();
        ntpSynchronize();
      }
    }
...
void alarmFeedback() {
  if (mode == 8) {
    ...
    String sMode = String(alarm1Mode);
    char bufferColor[6];
    char bufferHour[2];
    char bufferMinute[2];
    ...
    alarmRequest = "valid,r," + sMode + ",0x" + sColor + "," + sHour + ":" + sMinute +  "," + sWeek + "," + sTime;
  }
  else if (mode == 9) {
    //format strings for web ui
    String sMode = String(alarm2Mode);
    char bufferColor[6];
    char bufferHour[2];
    char bufferMinute[2];
    ...
    alarmRequest = "valid,s," + sMode + ",0x" + sColor + "," + sHour + ":" + sMinute +  "," + sWeek + "," + sTime;
  }
}
